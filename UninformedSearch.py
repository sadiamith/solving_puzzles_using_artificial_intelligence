...
class SearchTerminationRecord(object):
    ...
    def __init__(self, success=False, result=None, time=0, nodes=0, space=0, cutoff=False):
        ...
        # SearchNode: a node containing a goal state, or None if no solution found
        self.result = result
        # float: time was spent searching.  Not scientifically accurate, but good enough for fun
        self.time = time
        ...
        # Boolean: For IDS, True if depth limited search reach the depth limit before failing
        self.cutoff = cutoff
    ...
    def IDS(self, initial_state, search_type):
        ...
        while time < self._time_limit:
            ...
            if answer.success:
                ...
                nodes += answer.nodes
                # this could result in search that is substantial longer than the limit
                time += answer.time
                limit += 1
