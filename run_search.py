...
print('Estimated maximum time to solve',
      sys.argv[0], 'using strategies:', strategies, 'is', predicted_time, 'seconds')
...
for search_type in search_types:
    ...
    #     print('index :', 'target', 'size', 'depth', 'success', 'checked', 'time', 'nodes', 'space')
        ...
        for ex in examples:
            ...
            if solver == 'BFS':
                ...
                s = problem.create_initial_state()
                searcher = BlindSearch.Search(problem, timelimit=timelimit)
                answer = searcher.DepthLimitedSearch(
                    s, depth_limit, search_type)
            ...
            if answer.success:
                ...
                checked = (answer.result.state.puzzle ==
                           problem.goal_state.puzzle)
    ...
    #                   answer.time, answer.nodes, answer.space)
                ...
                ebf_stat.add(roots.eff_br_fact(
                    answer.nodes, answer.result.depth))
    ...
    #             print(ex, None, answer.success, None, answer.time, answer.nodes, answer.space, '*****')
        ...
        print()
        print('Summary for', solver, 'using', search_type,
              'search on data set', sys.argv[0])
...
print('Took', global_finish - global_start,
      'seconds (predicted', predicted_time, 'seconds)')
